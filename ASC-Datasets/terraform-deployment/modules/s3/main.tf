...
resource "aws_s3_bucket_logging" "this" {
  count  = length(keys(var.logging_config)) > 0 ? 1 : 0
  bucket = aws_s3_bucket.this.id

  target_bucket = var.logging_config["logging_bucket_name"]
  target_prefix = try(var.logging_config["prefix"], null)
}
...
resource "aws_s3_bucket_replication_configuration" "this" {
  count = length(try(keys(var.replication_configuration), {})) > 0 ? 1 : 0
...
data "aws_iam_policy_document" "combined" {
  ...
  source_policy_documents = compact([
    ...
    var.attach_policy ? data.aws_iam_policy_document.this[0].json : ""
  ])
}
...
resource "aws_s3_bucket_inventory" "this" {
  for_each = { for k, v in var.inventory_configuration : k => v }
  ...
  destination {
    bucket {
      bucket_arn = try(each.value.destination.bucket_arn, aws_s3_bucket.this.arn)
