...
cleanup() {
  ...
  rm -f kms-downloaded-outputs.tfvars
  rm -f "$LOCAL_STATE_FILE"
}
trap cleanup EXIT

echo "Downloading KMS keys output from $INFRA_REPO_NAME..."

# Path in S3 where the tfvars file is stored
S3_OBJECT_KEY="$INFRA_REPO_NAME-outputs.tfvars"
OUTPUTS_TFVARS_FILE="kms-downloaded-outputs.tfvars"

echo "Downloading KMS keys output from s3://$S3_TERRAFORM_ARTIFACTS_BUCKET_NAME..."

if ! aws s3 cp "s3://$S3_TERRAFORM_ARTIFACTS_BUCKET_NAME/$S3_OBJECT_KEY" "$OUTPUTS_TFVARS_FILE"; then
    error_exit "Source outputs not found at $S3_OBJECT_KEY. Please ensure the $INFRA_REPO_NAME workflow ran successfully."
fi

# Extract kms_keys map
KMS_KEYS=$(awk -F '=' '/^kms_keys/ {print $2}' "kms-downloaded-outputs.tfvars" | jq '.')
