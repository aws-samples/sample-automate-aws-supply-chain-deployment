...
if  ! jf rt download \
      --flat=false \
      --recursive=true \
      "$JFROG_ARTIFACTS_REPO/$INFRA_REPO_NAME/$ENVIRONMENT/reusable-outputs/$INFRA_REPO_NAME-outputs.tfvars" \
      "kms-downloaded-outputs.tfvars"; then
      error_exit "Source outputs not found. Please run the $INFRA_REPO_NAME workflow first."
fi

mv $INFRA_REPO_NAME/$ENVIRONMENT/reusable-outputs/* ./
